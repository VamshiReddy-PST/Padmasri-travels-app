...
// No standing vehicle-delete endpoint/button exists anywhere in the app -
// Sold Out (above) is the one and only way to retire a vehicle day-to-day,
// even for the Owner, and it's fully reversible. The one deliberate,
// narrowly-scoped exception is DELETE /api/vehicles/legacy-first-batch
// further down - a one-time, hardcoded-list cleanup action for 6 specific
// original placeholder vehicles the Owner explicitly asked to have
// permanently removed, not a general capability.
...
const SAMPLE_SEED_VEHICLE_REGS = [
  ...
  "TS 09 IY 2008", "TS 09 JB 2009", "TS 09 KE 2010", "TS 09 LH 2011", "TS 09 MK 2012", "TS 09 NN 2013",
  "TS 09 OQ 2014", "TS 09 PT 2015", "TS 09 QW 2016", "TS 09 RZ 2017", "TS 09 SC 2018", "TS 09 TF 2019",
  "TS 09 UI 2020", "TS 09 VL 2021", "TS 09 WO 2022", "TS 09 XR 2023", "TS 09 YU 2024", "TS 09 ZX 2025",
  "TS 09 AA 2026", "TS 09 BD 2027", "TS 09 CG 2028", "TS 09 DJ 2029", "TS 09 EM 2030", "TS 09 FP 2031",
  "TS 09 GS 2032", "TS 09 HV 2033", "TS 09 IY 2034", "TS 09 JB 2035", "TS 09 KE 2036", "TS 09 LH 2037",
  "TS 09 MK 2038", "TS 09 NN 2039", "TS 09 OQ 2040", "TS 09 PT 2041", "TS 09 QW 2042", "TS 09 RZ 2043",
...
// The very first 6 vehicles created when this app was set up (a distinct
// earlier batch from the other 49 sample vehicles - registrations 1001-1006
// rather than 2002-2050) - they picked up real-looking Company/Site/
// Supervisor/Driver/Route assignments along the way, which is exactly why
// they're being force-removed by exact registration rather than folded into
// the ordinary Sold Out cleanup above. Owner-only, one-time, explicitly
// requested - deliberately bypasses the usual "has expense/trip history"
// safety check that a general delete feature would need, since the Owner
// named these 6 by hand knowing that.
const LEGACY_FIRST_BATCH_VEHICLE_REGS = [
  "TS 09 AT 1001", "TS 09 BT 1002", "TS 09 CT 1003", "TS 09 DT 1004", "TS 09 ET 1005", "TS 09 FT 1006",
];
app.delete(
  "/api/vehicles/legacy-first-batch",
  requireAuth,
  requireRole("owner"),
  h(async (req, res) => {
    const regSet = new Set(LEGACY_FIRST_BATCH_VEHICLE_REGS);
    const candidates = db.vehicles.filter((v) => regSet.has(v.reg));
    const deletedRegs = candidates.map((v) => v.reg);
    const deleteIds = new Set(candidates.map((v) => v.id));
    db.vehicles = db.vehicles.filter((v) => !deleteIds.has(v.id));
    const alreadyGoneCount = LEGACY_FIRST_BATCH_VEHICLE_REGS.length - candidates.length;
    if (deletedRegs.length) {
